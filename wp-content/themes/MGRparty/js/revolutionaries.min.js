...
$(function(){function e(f,b){var a=$("#page-meet-the-blitizens").find(".filters .selected a").attr("href").substr(1),d=parseInt(c($(f),a+"_")),a=parseInt(c($(b),a+"_"));return d>a?1:-1}function c(a,b){for(var c=a.attr("className").split(" "),d,e;c.length;)d=c.pop(),(d=d.match(RegExp("^"+b+"(.+)")))&&(e=d[1]);return e}var g=BLITZ.utils.getparams(),a=$("#blitizens-grid"),d=$("#bio-wrapper"),k=$("#bio"),j=$("#page-life-at-blitz");$("#nav-slider").slider();$(".filters").delegate("li","mouseover",function(){$(this).addClass("hover").siblings().removeClass("hover")}).delegate("li",
...
$(this).addClass("hover").siblings().removeClass("hover")}).delegate(".blitizen","mouseout",function(){a.hasClass("detail")||$(this).removeClass("hover").siblings()}).delegate(".blitizen","click",function(){if(a.hasClass("detail"))return!1;var f=$(this).removeClass("hover"),b=a.find("li.blitizen:visible"),c=a.find("li.blitizen:visible:gt(11)"),e=b.index(this);location.hash="meet-the-blitizens:"+f.find("a").attr("id");a.data("cells",b);2>e%4?(a.addClass("left detail").removeClass("right"),c.hide(),
...
"click",function(){"#life-at-blitz"==$(this).attr("href")?j.find(".video").html('<iframe src="http://player.vimeo.com/video/20489659" width="640" height="360" frameborder="0"></iframe>'):j.find(".video iframe").remove()});if(g.hash){var h=g.hash.match(/:(.*)/);h?(h=h[1],d.find(".btn-close").click(),$("#page-meet-the-blitizens li a[href='#all']").click(),$("#"+h).click()):(document.location.hash=g.hash,"life-at-blitz"==g.hash&&j.find(".video").html('<iframe src="http://player.vimeo.com/video/20489659" width="640" height="360" frameborder="0"></iframe>'))}else document.location.hash=
"meet-the-blitizens",BLITZ.our_people.initial_page_load=!0;setTimeout(function(){$(window).hashchange(BLITZ.our_people.hashchange)},500)});
BLITZ.our_people={hashchange:function(){"#meet-the-blitizens"==document.location.hash&&$bio_wrapper.find(".btn-close").click();trackPage(location.pathname+location.hash)},populate_photos:function(e,c){var g=e.find("a").attr("id"),a=c.find(".blitizen:visible");visible_blitizens_len=a.length;for(var d=0;d<visible_blitizens_len;d++)a.eq(d).find("a:first .back").append($("<img />",{src:"/wp-content/themes/BLITZ/resources/img/our-people/BLITiZens/"+g+"_0"+(d+1)+".jpg"}));$.browser.webkit?BLITZ.our_people.transition_card(c,
